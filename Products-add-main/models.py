...
class ProductVariant(db.Model):
    ...
    def to_shopify_format(self):
        ...
        # IMPORTANT: Price has already been doubled in product_mapper.py (pence->pounds, then *2)
        # Do NOT double it again here - just use the price as-is
        variant = {
            'price': self.price,
            'requires_shipping': self.requires_shipping,
            'taxable': self.taxable,
        ...
        if self.compare_at_price:
            # Use compare_at_price as-is (already adjusted in product_mapper)
...
class AIProductVariant(db.Model):
    ...
    def to_shopify_format(self):
        ...
        # IMPORTANT: AI Product prices are copied from source products that already have 2x markup
        # Do NOT double them again - just use the price as-is
        variant = {
            'price': self.price,
            'requires_shipping': self.requires_shipping,
            'taxable': self.taxable,
        ...
        if self.compare_at_price:
            # Use compare_at_price as-is (already adjusted in source product)
            variant['compare_at_price'] = self.compare_at_price
        if self.sku:
            variant['sku'] = self.sku
        if self.barcode:
